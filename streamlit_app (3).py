...
def fetch_messages_from_github(url):
    ...
    try:
        ...
        log_message(f"Fetching messages from: {raw_url}")
        ...
        if response.status_code == 200:
            ...
            if content:
                ...
                # Remove empty lines and filter out very short lines
                messages = [line for line in lines if len(line) > 2]
                
                if messages:
                    log_message(f"✅ Successfully fetched {len(messages)} messages from GitHub")
                    return messages
                else:
                    log_message(f"❌ No valid messages found in file")
                    return []
            else:
                log_message(f"❌ Could not decode file content")
                return []
        else:
            log_message(f"❌ Failed to fetch: HTTP {response.status_code}")
            return []
    except Exception as e:
        log_message(f"❌ Error fetching from GitHub: {str(e)}")
        return []
...
if not st.session_state.logged_in:
    ...
    if user_config:
        ...
        with tab1:
            ...
            # GitHub Messages Section
            st.markdown("### 📦 Load Messages from GitHub")
            st.markdown("Click buttons below to fetch messages directly from GitHub:")
            
            col1, col2 = st.columns(2)
            
            # GitHub URL 1 - FYT9.txt
            with col1:
                st.markdown("#### 📄 Message Set 1")
                if st.button("📥 Fetch FYT9 Messages", key="fetch_fyt9", use_container_width=True):
                    with st.spinner("Fetching messages from GitHub..."):
                        messages = fetch_messages_from_github("https://github.com/JOHNSEENA1/e2e-rowedy--via/blob/main/FYT9.txt")
                        if messages:
                            messages_text = '\n'.join(messages)
                            ...
                            st.success(f"✅ Loaded {len(messages)} messages from FYT9.txt!")
                            st.rerun()
                        else:
                            st.error("❌ Failed to fetch messages. Check the URL or try again.")
            
            # GitHub URL 2 - Emoji File.txt
            with col2:
                st.markdown("#### 😊 Emoji Messages")
                if st.button("📥 Fetch Emoji Messages", key="fetch_emoji", use_container_width=True):
                    with st.spinner("Fetching emoji messages from GitHub..."):
                        messages = fetch_messages_from_github("https://github.com/JOHNSEENA1/e2e-rowedy--via/blob/main/Emoji%20File.txt")
                        if messages:
                            messages_text = '\n'.join(messages)
                            db.update_user_config(
                                st.session_state.user_id,
                            ...
                            st.success(f"✅ Loaded {len(messages)} emoji messages!")
                            st.rerun()
                        else:
                            st.error("❌ Failed to fetch emoji messages. Check the URL or try again.")
            
            # Custom GitHub URL input
            st.markdown("#### 🔗 Custom GitHub URL")
            custom_github_url = st.text_input(
                "Enter GitHub file URL",
                placeholder="https://github.com/username/repo/blob/main/messages.txt",
                help="Paste any GitHub raw file URL containing messages (one per line)"
            )
            
            if custom_github_url and st.button("📥 Fetch from Custom URL", key="fetch_custom", use_container_width=True):
                with st.spinner("Fetching messages from custom URL..."):
                    messages = fetch_messages_from_github(custom_github_url)
                    if messages:
                        messages_text = '\n'.join(messages)
                        db.update_user_config(
                            st.session_state.user_id,
                        ...
                        st.success(f"✅ Loaded {len(messages)} messages from custom URL!")
                        st.rerun()
                    else:
                        st.error("❌ Failed to fetch messages. Check the URL and try again.")
            
            st.markdown("---")
            
            # File upload as backup option
            st.markdown("### 📁 Upload Local File (Alternative)")
            st.markdown("If GitHub fetch doesn't work, upload a local file:")
            
            uploaded_file = st.file_uploader(
                "Choose a text file with messages (one message per line)",
                type=['txt', 'md', 'csv'],
                key="messages_uploader",
                help="Upload a text file where each line is a separate message"
            )
            
            if uploaded_file is not None:
                try:
                    file_content = uploaded_file.getvalue().decode("utf-8")
                    messages_from_file = [line.strip() for line in file_content.split('\n') if line.strip()]
                    st.success(f"✅ Loaded {len(messages_from_file)} messages from file!")
                    
                    with st.expander("📝 Preview Messages", expanded=False):
                        for i, msg in enumerate(messages_from_file[:20], 1):
                            st.text(f"{i}. {msg[:100]}{'...' if len(msg) > 100 else ''}")
                        if len(messages_from_file) > 20:
                            st.info(f"... and {len(messages_from_file) - 20} more messages")
                    
                    if st.button("📥 Use Uploaded Messages", key="use_uploaded"):
                        ...
                        st.success(f"✅ {len(messages_from_file)} messages loaded from file and saved!")
                        st.rerun()
                except Exception as e:
                    st.error(f"❌ Error reading file: {str(e)}")
            
            # Show current messages preview
            current_messages = [msg.strip() for msg in user_config['messages'].split('\n') if msg.strip()]
            if current_messages:
                with st.expander(f"📝 Current Messages ({len(current_messages)} messages)", expanded=False):
                    for i, msg in enumerate(current_messages[:20], 1):
                        st.text(f"{i}. {msg[:100]}{'...' if len(msg) > 100 else ''}")
                    if len(current_messages) > 20:
                        st.info(f"... and {len(current_messages) - 20} more messages")
            
            # Save other config button
            col1, col2 = st.columns(2)
            with col1:
                if st.button("💾 Save Configuration (Chat ID, Prefix, Delay, Cookies)", use_container_width=True):
                    final_cookies = cookies if cookies.strip() else user_config['cookies']
                    messages_text = user_config['messages']
                    db.update_user_config(
                        st.session_state.user_id,
                        chat_id,
                        name_prefix,
            ...
            with col1:
                if st.button("▶️ Start E2ee", disabled=st.session_state.automation_state.running, use_container_width=True):
                    ...
                    if current_config and current_config['chat_id']:
                        current_messages_check = [msg.strip() for msg in current_config['messages'].split('\n') if msg.strip()]
                        if not current_messages_check:
                            st.warning("⚠️ No messages found! Please load messages using GitHub buttons or file upload first.")
                        else:
